-- Fix expiration cron job to use club-based architecture
-- Replace references to removed crm_discount_id with c7_club_id
-- Handle Commerce7 (clubs) vs Shopify (promotions) appropriately
...
BEGIN
  ...
  LOOP
    BEGIN
      -- Get client info to determine CRM type
      SELECT cl.* INTO v_client
      FROM clients cl
      WHERE cl.id = v_enrollment.client_id;

      IF NOT FOUND THEN
        RAISE WARNING 'Client not found for enrollment %', v_enrollment.id;
        v_errors := v_errors + 1;
        CONTINUE;
      END IF;
      ...
      -- Queue CRM sync for membership cancellation
      -- Commerce7: Use club_id (from stage) - we'll cancel the membership via c7_membership_id in API
      -- Shopify: Will need promotion IDs from club_stage_promotions (handled in API endpoint)
      IF v_enrollment.crm_id IS NOT NULL THEN
...
COMMENT ON FUNCTION process_expired_enrollments() IS 'Processes expired club enrollments, updates customer flags, and queues membership cancellations for CRM sync. Commerce7 uses club memberships, Shopify uses promotions. Note: New memberships come from order webhooks, not cron jobs.';
