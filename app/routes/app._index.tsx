...
async function loadDashboardData(client: Client) {
  let tierCount = 0;
  let memberCount = 0;
  ...
  if (!client) {
    return {
      ...
      totalLoyaltyPoints,
      setupComplete: false,
      completionStatus: { clubsTiers: false, communication: false, marketing: false, hasMarketingEnabled: false },
  ...
  if (clubProgram?.club_stages?.length) {
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  // In React Router v7, return promises directly - no need for defer()
  return {
    client,
    session,
    dashboardData: loadDashboardData(client), // Return promise directly
  };
}
...
function DashboardContent({
  ...
  dashboardData: Awaited<ReturnType<typeof loadDashboardData>>;
}) {
  ...
  const { tierCount, memberCount, totalCustomers, totalLoyaltyPoints, setupComplete, completionStatus, communicationProvider, hasLoyaltyPoints } = dashboardData;
  ...
  const onClickNeighborly = () => {
    window.open('https://www.commerce7.com/partners/app?id=yno-neighborly', '_blank');
  }
        ...
        {!setupComplete ? (
          ...
          <Banner tone="success" title={`Welcome back, ${firstName}!`}>
            <BlockStack gap="200">
              ...
              <Text variant="bodyMd" as="p">
                By the way. How is your tasting room outreach program? Need some help tracking referral performance? Check out <Link onClick={onClickNeighborly}>Yno Neighborly</Link> in your Commerce7 app store.
              </Text>
            </BlockStack>
          </Banner>
        )}
...
export default function AppDashboard() {
  const { client, session, dashboardData } = useLoaderData<typeof loader>();
  const location = useLocation();
  const navigate = useNavigate();
